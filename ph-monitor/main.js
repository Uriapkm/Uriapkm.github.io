...
PHMonitor.prototype.init = function() {
...
PHMonitor.prototype.setupEventListeners = function() {
    // Theme toggle
...
PHMonitor.prototype.updateChartTheme = function() {
...
PHMonitor.prototype.refreshPorts = async function() {
    try {
...
PHMonitor.prototype.connect = async function() {
    try {
        this.updateStatus('connecting', 'Connecting...');
        console.log('Starting connection process...'); // Debug log
        ...
        try {
            if (portSelect.value === 'request' || portSelect.value === '') {
                console.log('Requesting port from user...'); // Debug log
                port = await navigator.serial.requestPort({
                    // Add filters for Arduino devices
                    filters: [
                        { usbVendorId: 0x2341 }, // Arduino SA
                        { usbVendorId: 0x2A03 }, // Arduino LLC
                        { usbVendorId: 0x1A86 }, // CH340 chipset
                        { usbVendorId: 0x0403 }  // FTDI chipset
                    ]
                });
            } else {
                console.log('Using previously selected port...'); // Debug log
                const ports = await navigator.serial.getPorts();
                port = ports[parseInt(portSelect.value)];
            }
        } catch (e) {
            console.error('Port selection error:', e);
            throw new Error(`Puerto no seleccionado o acceso denegado: ${e.message}`);
        }
        ...
        console.log('Port selected, attempting to open...'); // Debug log
        // Open port
        await port.open({ 
            baudRate: 9600,
            dataBits: 8,
        ...
        console.log('Port opened successfully'); // Debug log
        this.serialPort = port;
        
        // Ensure previous readers/writers are properly closed
        if (this.reader) {
            console.log('Cleaning up old reader...'); // Debug log
            await this.reader.cancel();
            this.reader.releaseLock();
        }
        if (this.writer) {
            console.log('Cleaning up old writer...'); // Debug log
            await this.writer.close();
            this.writer.releaseLock();
        }
        
        console.log('Getting reader and writer...'); // Debug log
        // Get new reader and writer
        this.writer = port.writable.getWriter();
        this.reader = port.readable.getReader();

        this.isConnected = true;
        this.updateStatus('connecting', 'Probando conexión...');
        this.updateUI();
        // Start reading data and wait for initialization
        console.log('Starting read loop...'); // Debug log
        this.startReading();
        
        console.log('Waiting for Arduino initialization...'); // Debug log
        ...
        this.showNotification('¡Conectado al medidor de pH!', 'success');
...
PHMonitor.prototype.disconnect = async function() {
    try {
        this.isConnected = false;
...
PHMonitor.prototype.startReading = async function() {
    try {
...
PHMonitor.prototype.processSerialData = function(line) {
    // Match different message formats
    const phMatch = line.match(/PH:([\d.]+)/);
    const currentPhMatch = line.match(/CURRENT_PH:([\d.]+)/);
    
    if (phMatch || currentPhMatch) {
        const rawPH = parseFloat(phMatch ? phMatch[1] : currentPhMatch[1]);
        console.log('Received pH reading:', rawPH); // Debug log
        this.processPHReading(rawPH);
    } else if (line.includes('PH_METER_READY')) {
        console.log('Device initialized successfully');
        this.showNotification('pH Meter ready for measurements', 'success');
    } else if (line.includes('ARDUINO_RESPONDING')) {
        console.log('Arduino connection test successful');
        this.showNotification('Connection test successful', 'success');
    } else if (line.includes('CALIBRATION_RESET')) {
        console.log('Calibration reset to defaults');
        this.showNotification('Calibration reset to defaults', 'success');
    } else if (line.startsWith('CALIBRATING:')) {
        const targetPH = line.split(':')[1];
        this.showNotification(`Calibrating for pH ${targetPH}...`, 'info');
    } else if (line.startsWith('CALIBRATION_COMPLETE:')) {
        const voltage = line.split(':')[1];
        this.showNotification(`Calibration complete (${voltage}V)`, 'success');
    } else if (line.startsWith('ERROR:')) {
        const error = line.split(':')[1].trim();
        console.error('Arduino error:', error);
        this.showNotification(error, 'error');
    } else {
        console.log('Received unhandled line:', line); // Debug log for other messages
    }
};

PHMonitor.prototype.calculateAverage = function(startTime, endTime) {
...
PHMonitor.prototype.processPHReading = function(rawPH) {
    // 1. Apply calibration
...
PHMonitor.prototype.sendCommand = async function(command) {
    if (this.writer) {
...
PHMonitor.prototype.startLogging = function() {
...
PHMonitor.prototype.readLoop = async function() {
...
PHMonitor.prototype.stopLogging = function() {
    this.isLogging = false;
...
PHMonitor.prototype.addCalibrationPoint = function() {
...
PHMonitor.prototype.computeCalibration = function() {
...
PHMonitor.prototype.resetCalibration = function() {
    this.calibrationPoints = [];
    this.slope = 1.0;
    this.offset = 0.0;
    this.updateCalibrationTable();
...
PHMonitor.prototype.updateCalibrationTable = function() {
...
PHMonitor.prototype.removeCalibrationPoint = function(index) {
...
PHMonitor.prototype.updateCalibrationUI = function() {
...
PHMonitor.prototype.updateChart = function() {
...
PHMonitor.prototype.updateDataTable = function() {
...
PHMonitor.prototype.updateStatistics = function() {
    if (this.phData.length === 0) {
...
PHMonitor.prototype.updateExportUI = function() {
...
PHMonitor.prototype.exportData = async function(format) {
...
PHMonitor.prototype.clearData = function() {
    if (this.phData.length === 0) {
...
PHMonitor.prototype.copyArduinoCode = function() {
...
PHMonitor.prototype.switchTab = function(tabId) {
    // Update tab buttons
...
PHMonitor.prototype.updateStatus = function(status, text) {
...
PHMonitor.prototype.updateUI = function() {
    const isConnected = this.isConnected;
    const isLogging = this.isLogging;
    const hasData = this.phData.length > 0;
    
    // Helper function to safely update element
    const safeSetDisabled = (id, value) => {
    ...
    // Connection controls
    safeSetDisabled('connectBtn', isConnected);
    safeSetDisabled('disconnectBtn', !isConnected);
    safeSetDisabled('portSelect', isConnected);
    
    // Logging controls
    safeSetDisabled('startLoggingBtn', !isConnected || isLogging);
    safeSetDisabled('stopLoggingBtn', !isLogging);
    safeSetDisabled('intervalInput', isLogging);
    
    // Calibration controls
    safeSetDisabled('addCalPointBtn', !isConnected);
    safeSetDisabled('resetCalBtn', this.calibrationPoints.length === 0);
    
    // Data export
    safeSetDisabled('exportCSVBtn', !hasData);
    safeSetDisabled('exportExcelBtn', !hasData);
    ...
    // Update export measurements count
    document.getElementById('exportTotalMeasurements').textContent = this.phData.length;
};

PHMonitor.prototype.toggleTheme = function() {
    const isDark = document.documentElement.dataset.theme === 'dark';
...
PHMonitor.prototype.loadTheme = function() {
...
PHMonitor.prototype.showNotification = function(message, type = 'info') {
...
PHMonitor.prototype.getNotificationIcon = function(type) {
    const icons = {
